...
import { useOrganizationSettings } from "@/providers/organization-settings-provider";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function InvoiceForm({ initialData }: InvoiceFormProps) {
  ...
  const defaultTaxRate = orgSettings.taxSettings.enableTaxCalculation ? orgSettings.taxSettings.taxRate : 0;
  ...
  const onSubmit = async (data: InvoiceFormValues, action: "draft" | "send") => {
    ...
    try {
      ...
      const result = isEditMode 
        ? await updateInvoice(initialData._id, invoiceData, pathname)
        : await createInvoice(invoiceData, pathname);

      if (result.error) {
        toast.error(result.error);
      } else {
        toast.success(`Invoice ${isEditMode ? "updated" : action === "draft" ? "saved as draft" : "created and sent"}`);
        router.push(`/${params.organizationId}/dashboard/${params.userId}/sales/invoices`);
      }
    } catch (error) {
  ...
  const handleWhatsAppShare = () => {
    const message = `*INVOICE ${form.watch("invoiceNumber")}*%0A%0A` +
      `Customer: ${selectedCustomer?.name}%0A` +
      `Company: ${selectedCustomer?.company}%0A%0A` +
      `Invoice Date: ${format(form.watch("invoiceDate"), "PPP")}%0A` +
      `Due Date: ${format(form.watch("dueDate"), "PPP")}%0A%0A` +
      `*Total Amount: GH₵ ${calculateTotal().toFixed(2)}*%0A%0A` +
      `Thank you for your business!%0A%0A` +
      `_Generated by PayFlow_`;
    
    window.open(`https://wa.me/?text=${message}`, "_blank");
    toast.info("Tip: Download PDF first, then attach it in WhatsApp");
  };
        ...
        <div ref={printRef} className="space-y-8 print:p-12 bg-white">
          ...
          <div className="print:hidden">
            <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white p-6 rounded-lg mb-6">
              ...
              <p className="text-emerald-100 text-sm mt-1">#{form.watch("invoiceNumber")}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="text-gray-900 font-medium">{form.watch("invoiceNumber")}</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="font-semibold text-gray-700">Invoice Date:</span>
                <span className="text-gray-900">{format(form.watch("invoiceDate"), "PPP")}</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="font-semibold text-gray-700">Due Date:</span>
                <span className="text-gray-900 font-medium">{format(form.watch("dueDate"), "PPP")}</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="font-semibold text-gray-700">Payment Terms:</span>
                <span className="text-gray-900">{paymentTermsOptions.find(t => t.value === form.watch("paymentTerms"))?.label}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end mt-8">
            <div className="w-80">
              <div className="space-y-3 bg-gray-50 p-6 rounded-lg border border-gray-200">
                ...
                <div className="flex justify-between text-sm py-2">
                  <span className="text-gray-600">Tax ({taxType}):</span>
            ...
            <div className="mt-8 space-y-4">
              {form.watch("notes") && (
                <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
                  <p className="font-semibold text-blue-900 mb-2 text-sm">Notes:</p>
          ...
          <div className="print:block hidden mt-12 pt-8 border-t-2 border-gray-200">
            <div className="text-center">
              <p className="text-gray-600 text-sm font-medium">Thank you for your business!</p>
              <p className="text-gray-500 text-xs mt-2">This invoice was generated by PayFlow - Professional Payroll & Accounting</p>
