...
import { createEquityTransaction } from "@/lib/actions/equity-transaction.action";
...
export function EquityForm() {
  ...
  const onSubmit = async (data: EquityFormValues) => {
    ...
    try {
      const result = await createEquityTransaction(data);

      if (result.error) {
        toast.error(result.error);
      } else {
        toast.success("Equity transaction created successfully");
