...
async function _getDashboardStats(user: any) {
  try {
    await connectToDB();
    ...
    const pendingInvoicesAmount = pendingInvoices.reduce((sum, inv) => sum + (inv.total - inv.paidAmount), 0);

    // Unpaid Bills
    const unpaidBills = bills.filter((bill) => bill.status === "pending" || bill.status === "overdue");
    const unpaidBillsAmount = unpaidBills.reduce((sum, bill) => sum + (bill.total - bill.paidAmount), 0);
    ...
    const customerStats = await Invoice.aggregate([
      { $match: { organizationId: user.organizationId, del_flag: false } },
    ...
    const vendorStats = await Bill.aggregate([
      { $match: { organizationId: user.organizationId, del_flag: false } },
      { $group: { _id: "$vendorId", totalAmount: { $sum: "$total" } } },
      { $sort: { totalAmount: -1 } },
      { $limit: 5 },
    ]);
